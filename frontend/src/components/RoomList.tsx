...
const RoomList: React.FC<RoomListProps> = ({ rooms, onJoinRoom, onCreateRoom, onRefreshRooms, onChangeNickname, nickname }) => {
  ...
  return (
    <div className="w-full max-w-6xl flex flex-col gap-8">
      {/* 닉네임 변경 모달 */}
      {showNicknameEdit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setShowNicknameEdit(false)}>
          <div className="panel-border bg-background-dark/95 p-6 rounded-lg w-80 space-y-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="panel-border bg-slate-900/50 p-6 rounded-xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                {gameTypes.map(type => (
                  <option key={type.value} value={type.value} className="bg-background-dark">{type.label}</option>
                ))}
              </select>
            </div>

            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-primary/70 uppercase tracking-widest">장르/카테고리</label>
              <select
                className="w-full bg-slate-950 border border-primary/30 rounded-lg p-3 text-white focus:border-primary outline-none appearance-none"
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                disabled={gameType === 'CS'}
              >
                {categories.map(cat => (
            ...
            <div className="space-y-2">
              ...
              >
                {[2,3,4,5,6,7,8].map(n => (
                  <option key={n} value={n} className="bg-background-dark">{n}명</option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                {songCountOptions.map(n => (
                  <option key={n} value={n} className="bg-background-dark">{n}문제</option>
                ))}
              </select>
            </div>
          </div>
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <div className="flex flex-col gap-1 min-w-0">
                    <h3 className="text-lg font-bold text-white uppercase truncate group-hover:text-primary transition-colors">{room.name}</h3>
                    {isPlaying && (
