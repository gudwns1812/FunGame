 ...
 */
const TopBar: React.FC<TopBarProps> = ({ title, right, onLogoClick }) => {
  return (
    <header className="shrink-0 bg-paper border-b-[4px] border-ink px-3 sm:px-5 h-14 flex items-center justify-between gap-3">
      <div className="flex items-center gap-2.5 min-w-0">
        <button
          ...
          className="flex items-center gap-2.5 min-w-0 disabled:cursor-default">
          ...
          <span className="px-title text-base sm:text-lg">펀게임</span>
        </button>

        {title && (
          <>
            <span className="w-2 h-2 bg-ink shrink-0" />
            <span className="px-title text-sm text-ink-soft truncate">{title}</span>
          </>
        )}
      </div>
