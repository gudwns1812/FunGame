...
const MyPage: React.FC = () => {
  ...
  const handleRequestPromotion = async () => {
    ...
    try {
      ...
      notifyError(err.response?.data?.error?.message || '승급 신청 중 오류가 발생했습니다.');
    } finally {
  ...
  return (
    <div className="app-frame">
      ...
      <main className="flex-1 min-h-0 scroll-y custom-scrollbar p-4 sm:p-6">
        <div className="w-full max-w-2xl mx-auto flex flex-col gap-4 animate-pop">
          {/* 프로필 */}
          <div className="px-card p-4 flex items-center gap-4">
            <span className="w-16 h-16 shrink-0 border-[3px] border-ink bg-cherry flex items-center justify-center">
              <span className="px-title text-2xl text-white">
