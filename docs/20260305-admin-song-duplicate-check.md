# AdminSongPage 노래 제목 중복 체크 기능 추가 계획
...
## 4. 실행 순서
1. `AdminSongPage.tsx`에 상태 변수 추가.
2. `handleCheckDuplicate` 함수 구현.
3. UI 레이아웃 수정 및 메시지 렌더링 로직 추가.
4. 제목 변경 시(`onChange`) 중복 체크 상태 리셋 로직 추가.
5. 최종 동작 확인 (API 호출 및 스타일 적용 확인).
