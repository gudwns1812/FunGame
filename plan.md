...
- **2단계와 3단계를 한 커밋으로 묶었다.** 목차의 관리자 항목과 `MyPage` 의 `inquiries` 경로가
  `ReportAdminSection` 없이는 존재할 수 없어, 나누면 중간 커밋이 깨진 화면이 된다.
- **`ReportStatus` 는 늘리지 않았다.** 상태를 넷으로 넓히자고 적었지만 둘로 정했다.
- **`R004`(답변 내용 누락) · `R005`(신고 없음) 를 새로 만들었다.** 답변이 비었을 때 "신고 내용을
  입력해주세요" 라고 말하는 건 틀린 안내다.
- **`ReportSummary` 에 신고 번호 줄을 더했다.** 디스코드 알림만 보고 관리 화면에서 그 건을 찾을 수 있어야 한다.
