...
**마킹은 순서를 만든다.** V9, V10 을 운영 이력에 기록하면 적용된 최고 버전이 10 이 된다. 그보다 낮은 번호의 마이그레이션이 아직 대기 중이면 flyway 가 out-of-order 로 거부한다.

```
Detected resolved migration not applied to database: 8.
```

새 마이그레이션(대기 테이블)이 V8 이었다가 실제로 이 문제를 밟았다. V12 로 올려 해결했다. 앞으로도 마킹된 버전보다 낮은 번호를 쓰면 안 된다.
