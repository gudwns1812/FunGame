...
특정 방에서 플레이어가 나갑니다. 대기 중인 방뿐 아니라 **게임이 진행 중인 방에서도 호출할 수 있습니다.**
...
게임 중에 호출하면 남은 사람들은 유예 없이 곧바로 이탈을 통보받고, 이탈자는 랭킹(`/play/rank`, 최종 결과)에서 빠집니다.
방장이 나가면 남은 사람 중 한 명에게 방장이 위임되고, 마지막 사람이 나가면 방과 게임 세션이 함께 정리됩니다.

이탈자는 **참가자 자격을 유지**하므로 진행 중인 그 방에 다시 입장할 수 있고, 이때 점수와 현재 라운드를 이어받습니다.
이탈로 스킵 정족수(`max(1, 남은 인원 - 1)`)가 채워지면 해당 라운드는 타임아웃을 기다리지 않고 그 자리에서 끝납니다.
